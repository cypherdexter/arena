...
async function loadMarkets(){
  const d=await getJ("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=130&page=1&price_change_percentage=7d,30d,90d,1y");
...
async function loadSectors(btc,prot){
  ...
  for(const [id,name] of SECTORS){
    ...
    const u=`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&category=${id}&order=market_cap_desc&per_page=25&page=1&price_change_percentage=7d,30d,90d`;
    ...
    const b90=btc.price_change_percentage_90d_in_currency;const r90v=d.map(c=>c.price_change_percentage_90d_in_currency).filter(x=>x!=null);
    ...
    let phase="Neutra";
    if(r90!=null&&r90>50&&r7<0)phase="Affollata";
    else if(r30>8&&r7>=-1)phase="In rotazione";
    else if(r7>3&&r30<=8&&(r90==null||r90<15))phase="Emergente";
...
const API={hook,KB,STABLE,SECTORS,sleep,fmt,px,pc,getJ,loadMarkets,loadLlama,candles,loadSectors,tech,fund,decide,analyze,applyRun,computeRegime,portfolioWeights,monteCarlo,kellyMult,ema,sma,med,TARGET};

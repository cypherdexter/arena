...
function paperStep(P,RES,PREV,REG,prices,full){
  ...
  const btc=RES.find(r=>r.id==="bitcoin");if(!P)P=paperInit(btc?btc.price:prices.bitcoin);
  ...
  const exp=(REG&&REG.exposure||70)/100,hot=REG&&REG.hot;
  ...
  const needRebal=!P.lastRebal||(Date.now()-Date.parse(P.lastRebal))/864e5>=c.rebalDays||(P.lastExp!=null&&Math.abs(exp*100-P.lastExp)>=15);
  if(needRebal){for(const id of coreIds){const target=eq*exp*c.coreW*cw[id]/cs;const p=P.pos[id];const cur=p?p.qty*prices[id]:0;const diff=target-cur;
    ...
    P.lastRebal=now;P.lastExp=exp*100}
  ...
  if(!hot){const cands=RES.filter(r=>conf(r.id)==="COMPRA"&&!P.pos[r.id]&&!coreIds.includes(r.id)&&prices[r.id]&&r.t&&r.t.E50).sort((a,b)=>b.total-a.total);
    for(const r of cands){if(Object.keys(P.pos).filter(id=>!P.pos[id].core).length>=c.satMax)break;
      const room=eq*exp*c.satBudget-satVal;const size=Math.min(eq*c.satCap,room,P.cash*0.98);if(size<eq*0.02)break;
...
const API={hook,PAPER_CFG,paperInit,paperEquity,paperStep,KB,STABLE,SECTORS,sleep,fmt,px,pc,getJ,loadMarkets,loadLlama,candles,loadSectors,tech,fund,decide,analyze,applyRun,computeRegime,portfolioWeights,monteCarlo,kellyMult,ema,sma,med,TARGET};

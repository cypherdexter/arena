...
(function(root){
const hook={status:()=>{},progress:()=>{},cgKey:null};
...
/* ---------- data ---------- */
async function getJ(u,ms=20000){const c=new AbortController();const t=setTimeout(()=>c.abort(),ms);const h={};if(hook.cgKey&&u.includes("api.coingecko.com"))h["x-cg-demo-api-key"]=hook.cgKey;try{const r=await fetch(u,{signal:c.signal,headers:h});if(!r.ok)throw new Error(r.status);return await r.json()}finally{clearTimeout(t)}}
...
function decide(c,f,t){
  ...
  const techWeak=t.score<60;
  const half=f.dil<0.5; // forte diluizione: si compra a metà dimensione, non si esclude
  if((fWeak&&techWeak)||(t.down&&t.rs90!=null&&t.rs90<-15))state="EVITA";
  else if((fOK||(!fWeak&&t.score>=85))&&strong&&entry&&t.rsi<76)state="COMPRA";
  else if(f.score>=42)state="ATTENDI";
  else state="EVITA";
  // fundamentals text
  ...
  if(state==="COMPRA"&&half)trig+=" Diluizione alta: dimensione dimezzata.";
  if(state==="EVITA")trig=fWeak?"Fondamentali insufficienti: fuori dall'universo finché diluizione o ricavi non migliorano.":"Debolezza strutturale contro BTC: tenere BTC rende di più con meno rischio.";
...
const PAPER_CFG={start:10000,fee:0.0026,slip:0.0010,satCap:0.08,satMax:6,satBudget:0.55,coreW:0.35,tp:[[2,0.20],[3,0.25],[5,0.30]],rebalDays:30,rebalDrift:0.02,rotGap:15};
...
function paperStep(P,RES,PREV,REG,prices,full){
  ...
  if(!hot){const cands=RES.filter(r=>conf(r.id)==="COMPRA"&&!P.pos[r.id]&&!coreIds.includes(r.id)&&prices[r.id]&&r.t&&r.t.E50).sort((a,b)=>b.total-a.total);
    // rotazione per merito: un candidato che supera di ≥15 punti il satellite più debole (non in utile oltre il 10%) lo sostituisce
    for(const r of cands){const satsNow=Object.keys(P.pos).filter(id=>!P.pos[id].core);
      const room=eq*exp*sty.sat-satVal;if(satsNow.length<c.satMax&&room>=eq*0.02)break;
      const weak=satsNow.map(id=>({id,p:P.pos[id],r:byId[id]})).filter(x=>x.r&&prices[x.id]&&(prices[x.id]/x.p.cost-1)<0.10).sort((a,b)=>a.r.total-b.r.total)[0];
      if(!weak||r.total-weak.r.total<c.rotGap)break;
      const t=paperTrade(P,weak.id,weak.p.sym,"SELL",weak.p.qty,prices[weak.id],`rotazione: sostituito da ${r.sym} (tot. ${r.total} vs ${weak.r.total})`,now);
      if(t){ev.push(t);satVal-=t.value}}
    for(const r of cands){if(Object.keys(P.pos).filter(id=>!P.pos[id].core).length>=c.satMax)break;
      const room=eq*exp*sty.sat-satVal;let size=Math.min(eq*c.satCap,room,P.cash*0.98);if(r.half)size=Math.min(size,eq*c.satCap/2);if(size<eq*0.02)break;
      const qty=size/prices[r.id]/(1+c.slip)/(1+c.fee);const t=paperTrade(P,r.id,r.sym,"BUY",qty,prices[r.id],`segnale confermato (tot. ${r.total})${r.half?", mezza posizione per diluizione":""}`,now);
